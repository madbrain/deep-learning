...
export class MatrixValueFactory {
    public matrix(elements: number[][]): Matrix { return new MatrixImpl(elements); }
    public random(height: number, width: number): Matrix {
        let newElements = [];
    ...
    public constant(value: number): Matrix { return new MatrixImpl([[value]]); }
